            ...
            <div class="bg-white p-4 rounded-md">
                <form action="{{ route('admin.post.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-3 mb-4">
                        ...
                        <x-textarea id="body" class="block mt-1 w-full" type="text" name="body"   autofocus>{{old('body')}}</x-textarea>
                    </div>

                    <div class="space-y-3 mb-4">
                        <x-label for="thumbnail" class="text-sm" :value="__('Image')" />
                        <x-input type="file" name="thumbnail" :value="old('thumbnail')"
                    ...
                    <div class="space-y-3 mb-4">
                        ...
                        <select name="tag_id[]"  multiple="multiple"  id="tag" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            ...
                            @foreach (\App\Models\Tag::all() as $tag)
                                <option value="{{$tag['id']}}" {{old('tag_id') == $tag['id'] ? 'selected' : ''}}  >{{ucwords($tag['name'])}}</option>
                            @endforeach
                        </select>
                    </div>
